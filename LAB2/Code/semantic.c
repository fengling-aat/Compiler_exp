...
int conflict_between_functions(struct FUNCTION* funca,struct FUNCTION* funcb){
    ...
    if(fieldcmp_forfunc(funca->field,funcb->field)!=0) return 1;
...
//0表示找到，1表示未找到
int find_domain_in_struct(Type type,struct Node* now){
    //printf("afind_domain_in_struct\n");
    FieldList temp_field=type->u.structure;
    while(temp_field!=NULL){
        if(strcmp(now->char_name,temp_field->name)==0) return 0;
...
Type Exp(struct Node* now){
    ...
    if(strcmp(fir_bro->name,"ASSIGNOP\0")==0){
        ...
        else if(strcmp(child_child->name,"ID\0")==0);
        else if(strcmp(child_child->name,"Exp\0")==0&&strcmp(child->brother->name,"LB\0")==0);
        else if(strcmp(child_child->name,"Exp\0")==0&&strcmp(child->brother->name,"DOT\0")==0);
        else{
    ...
    if(strcmp(fir_bro->name,"DOT\0")==0){
        ...
        int has_domain=find_domain_in_struct(return_type,fir_bro->brother);
        if(has_domain==1){
